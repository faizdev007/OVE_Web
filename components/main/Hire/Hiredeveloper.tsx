        ...
        <div className="relative py-12 sm:px-6 lg:px-8 mx-auto flex flex-col gap-10 dark:bg-black">
            ...
            <div className="prose prose-lg prose-invert max-w-none flex gap-2 relative">
                <div className="sticky top-20 self-start hidden lg:block w-1/4 pr-8">
                    <h2 className="text-2xl font-bold mb-8 text-gray-700 dark:text-white">Guide Topics</h2>
                    <div className="list-disc list-inside">
                        {hireDev?.whdsList?.map((item: any, index: number) => (
                            <p className={`p-2 ${currentTopicIndex === index ? "bg-oveblue/20 dark:bg-oveblue rounded font-bold" : ''}`} key={index}><a onClick={()=>setCurrentTopicIndex(index)} href={`#${item.topicTitle.toLowerCase().replace(/\s+/g, '-')}`} className="text-gray-600 dark:text-white">{item.topicTitle}</a></p>
