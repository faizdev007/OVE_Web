    ...
    return (
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 1, ease: "easeOut" }} viewport={{ once: true }} className="relative dark:bg-gray-900 dark:py-12 px-4 sm:px-6 lg:px-8">
            <div className="relative z-20 mx-auto">
                <motion.h2 initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 1.5, ease: "easeOut" }} viewport={{ once: true }} className="xl:text-4xl md:text-3xl text-2xl font-bold text-center text-white mb-12">10x Faster Hiring : 50 Concurrent Interviews :  AI Engine Handles First-Round Screening</motion.h2>
                <p className="text-lg text-white text-center">Screen 100+ candidates in hours, interview only the best. Our AI handles the first round so you don't have to.</p>
                <h3 className="font-semibold mb-2  text-white text-center">Why Our AI Screening Changes Everything:</h3>
                <div className="text-white flex lg:flex-row flex-col pt-10">
                ...
                <div className="mt-10">
                    <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 1.5, ease: "easeOut" }} viewport={{ once: true }}>
                        <div className='w-full flex justify-center mt-10'>  
                            <a href={'/contact-us'} className='lg:w-1/3 w-min cursor-pointer text-nowrap text-center hover:bg-btnblue/90 drop-shadow shadow hover:shadow-xl bg-btnblue text-white lg:p-3 p-2 xl:text-xl text-nowarp font-bold rounded'>{'Book A Free Consultation Today!'}</a>
                        </div>
                    </motion.div>
                </div>
            </div>
